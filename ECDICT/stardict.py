...
if __name__ == '__main__':
    ...
    # test3()
    def start_convert_csv2sqllite():
        csv_name = 'D:\\any_code\EnglishRBB\ECDICT\stardict.csv'
        src_name = 'D:\\any_code\EnglishRBB\ECDICT\stardict.db'
        convert_dict(src_name, csv_name)
    m = 'sad '.strip()
    MyDict = StarDict('D:\\any_code\EnglishRBB\ECDICT\stardict.db')
    res = MyDict.query('all')
    for k, v in res.items():
        print(f'{k}:{v}')
    res = MyDict.query_batch(['he', 'his', 'gave'])
    for i in res:
        print('-----------')
        for k,v in i.items():
            print(f'{k}:{v}')
